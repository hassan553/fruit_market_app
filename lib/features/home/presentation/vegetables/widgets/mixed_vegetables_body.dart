...
class MixedVegetablesBody extends StatelessWidget {
  const MixedVegetablesBody({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<VegetablesBloc, VegetablesState>(
      builder: (context, state) {
        switch (state.mixedVegetablesState) {
          case RequestState.loaded:
            return Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                FruitTitleTextWidget(
                  priceOff: state.mixedVegetablesCollection.priceOff,
                  subTitle: state.mixedVegetablesCollection.subTitle,
                  title: state.mixedVegetablesCollection.title,
                ),
                SizedBox(
                  height: 8.sp,
                ),
